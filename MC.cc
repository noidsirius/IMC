...
namespace _MC {
	...
	MC::MC(AnyOption* opt1) {
		...
		communities_AM = new HashTreeCube*[5];
		communities_AM[0] = new HashTreeCube(89041);
		all_communities = new HashTreeMap<UID, Community* > (89041);
		communities_by_depth = new HashTreeMap<int, vector<Community*>* >(89041);
	...
	float MC::mineSeedSetPlus() {
		...
		while (curSeedSet.size() < budget && totalCov < totalUserNum) {
			cout << "In other pass " << curSeedSet.size() <<endl;
			Gains::iterator iter = gainTable.end();
			iter--;
...
void MC::openOutputFiles() {
	...
	int celfPlus = strToInt(opt->getValue("celfPlus"));
	if (celfPlus == 1) {
		filename = outdir + "/" + m + "_CelfPlus_" + algorithm + ".txt";
	} else {
		filename = outdir + "/" + m + "_Celf_" + algorithm + ".txt";
	}
...
void MC::readInputData(float alpha) {
	...
	if (myfile.is_open()) {
		while (! myfile.eof() )	{
			...
			if (u1 == u2) continue;
			this->max_node_id = max(u1, max(u2, max_node_id));
			...
			if(all_communities->find(u1) == NULL)
			createCommunity(u1, 0);
			if(all_communities->find(u2) == NULL)
			createCommunity(u2, 0);
	...
	this->numEdges = edges;
	this->last_community_id = this->max_node_id;
...
void MC::createCommunity(UID id, int depth){
	Community* c = new Community(id, depth);
	all_communities->insert(id, c);
	if(communities_by_depth->find(depth) == NULL)
	communities_by_depth->insert(depth, new vector<Community*>());
	vector<Community*> *depth_vector = communities_by_depth->find(depth);
	depth_vector->push_back(c);
}

void MC::makeCommunities(){
	for(UserList::iterator i = users.begin(); i != users.end(); i++) {
		Community* c = all_communities->find(*i);
		assert(c != NULL);
		if(c->parent == NULL)
		{
			last_community_id++;
			Community* parent_com = new Community(last_community_id, 1);
			c->setParent(parent_com);
			FriendsMap* neighbors = AM->find(*i);
			if (neighbors != NULL) {
				for (FriendsMap::iterator j = neighbors->begin(); j != neighbors->end(); ++j) {
					UID u = j->first;
					Community* neighbor_com = all_communities->find(u);
					assert(neighbor_com != NULL);
					if(neighbor_com->parent == NULL){
						neighbor_com->setParent(parent_com);
						FriendsMap* neighbors2 = AM->find(u);
						if (neighbors2 != NULL) {
							for (FriendsMap::iterator k = neighbors2->begin(); k != neighbors2->end(); ++k) {
								UID w = k->first;
								Community* neighbor_com2 = all_communities->find(w);
								assert(neighbor_com2 != NULL);
								if(neighbor_com2->parent == NULL){
									neighbor_com2->setParent(parent_com);
								}
							}
						}
					}
